...
export class MapComponent implements OnInit, OnChanges {
  ...
  constructor(
    private loadRadioService: LocationRadiosService,
  ...
  async loadRadios() {
    if (this.map == undefined) {
      ...
      if (
        this.chargeCountry != this.actualCountry ||
  ...
  getFavicon(station: Station) {
    return station!.favicon.length > 0
      ? station?.favicon
  ...
  async loadMarkers() {
    if (this.markerList.length === 0) {
      await this.loadRadioService
        .loadRadiosInCords(this.actualCountry)
        .then(() => {
