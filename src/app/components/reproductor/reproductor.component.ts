...
export class ReproductorComponent implements AfterViewInit {
  ...
  openBar() {
    this.volumeBar()
      ? document
          ...
          ?.classList.replace('openBar', 'closeBar')
      : document
          .getElementById('volumeBar')
          ?.classList.replace('closeBar', 'openBar');
    this.volumeBar.update(() => (this.volumeBar() ? false : true));
  }
  updateModeCar() {
    this.modeCar()
      ? this.modeCar.update(() => false)
  ...
  setState() {
    if (
      this.reproductorService.actualStation != undefined &&
      this.reproductorService.state()
    ) {
