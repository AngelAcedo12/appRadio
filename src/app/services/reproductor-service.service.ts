...
export class ReproductorServiceService {
  ...
  async play(urlSound: string, station: Station) {
    ...
    await this.audio.play().then(() => {
      this.stationLoading.update(() => true);
    });
  ...
  loadStationInLocalStorage() {
    var station: ModelSaving = JSON.parse(
      localStorage.getItem('actualStation')!
    );
  ...
  public addListertToAudio() {
    ...
    this.audio.addEventListener('error', (error) => {
      ...
      if (this.numberRetries == 3) {
        this.notificationService.openSnackBar({
          message:
            'Error al reproducir la estación, prube de nuevo más tarde...',
          duration: 2000,
          closeMessage: 'Cerrar',
        });
      }
  ...
  private addToHistory(history: DtoHistorySaving) {
    ...
    this.http
      .post<any>(`${enviroment.base_url_local}api/user/history`, {
        headers: headers,
        body: body,
      })
      .subscribe((res) => {});
